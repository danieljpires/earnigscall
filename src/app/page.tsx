    ...
    try {
      ...
      if (data.report) {
        setReport(data.report);
          ...
          // 4. Phase 2: Parallel Synthesis + Batch Q&A Extraction
          if (data.report.isPartial) {
            setLoadingStep(5); // "Processamento Paralelo Ativo..."
            
            const tasks = [];

            // TASK A: Launch Synthesis
            tasks.push((async () => {
              console.log("[Front-end] Starting parallel synthesis...");
              try {
                const synthRes = await fetchWithRetry("/api/analysis/synthesis", {
                  method: "POST",
                  headers: { "Content-Type": "application/json" },
                  body: JSON.stringify({
                    ticker,
                ...
                if (synthRes.ok) {
                  ...
                  if (synthData.synthesis) {
                    setReport((prev: any) => prev ? { 
                      ...prev, 
                      geminiAnalysis: { ...prev.geminiAnalysis, ...synthData.synthesis } 
                    } : prev);
                  }
                }
              } catch (e) {
            ...
            // TASK B: Batch Q&A Extraction
              const totalChunks = data.report.geminiAnalysis?.chunkCount || 0;
            if (totalChunks > 0) {
              tasks.push((async () => {
                const BATCH_SIZE = 2; // More stable for Free Tier
                for (let i = 0; i < totalChunks; i += BATCH_SIZE) {
                  ...
                  for (let j = 0; j < BATCH_SIZE && (i + j) < totalChunks; j++) {
                    ...
                    currentBatch.push((async () => {
                      try {
                        console.log(`[Front-end] Extracting Q&A chunk ${chunkIdx + 1}/${totalChunks}...`);
                        ...
                        console.error(`Failed to load Q&A chunk ${chunkIdx}:`, qaErr);
                      }
                    })());
                  }
                  ...
                  if (totalChunks > BATCH_SIZE && (i + BATCH_SIZE) < totalChunks) {
                    await new Promise(r => setTimeout(r, 600));
                  }
                  
                  setProgress((prev) => Math.min(prev + (10 * (currentBatch.length / totalChunks)), 100));
                }
              })());
            }
            ...
            <div>
              <h1 className="text-xl font-bold tracking-tight text-gray-900 dark:text-gray-50">
                ...
                <p className="text-[10px] text-blue-600 dark:text-blue-400 mt-0.5 flex items-center gap-1 font-medium bg-blue-50 dark:bg-blue-900/30 px-2 py-0.5 rounded-full">
                  ...
                  Gemini 1.5 Flash Turbo (v9.1 - Resilient)
                </p>
            </div>
          </div>
