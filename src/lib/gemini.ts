...
async function generateSynthesis(
  ...
  const prompt = `
  Analyze the ${ticker} earnings call and provide a PROFESSIONAL and DEEP synthesis for investors. 
  Translate ALL fields to ${languageName}. 
  MANDATORY: Use the exact financial dialect and vocabulary of ${languageName}.
  
  CURRENT CALL TRANSCRIPT SNIPPET (First 150k chars):
  ...
  1. Positives (Pontos Positivos): Achievements, growth, margin improvements, or successful initiatives mentioned by management.
  2. Drivers to Watch (Drivers a Acompanhar): Key variables, market conditions, or upcoming events that will drive stock performance.
  3. Management Outlook (Outlook da Administração): Specific guidance, future expectations, and strategic trajectory shared during the call.

  STRICT: Be professional and comprehensive. Ensure the "positives" and "driversToWatch" arrays are well-populated with actual facts from the call.
  
  Output JSON format:
  {
    "executiveSummary": "1-2 paragraphs of high-impact investor value.",
    "managementOutlook": "Detailed but concise strategic outlook and guidance.",
    "positives": ["3-6 precise positive points/achievements"],
    "negatives": ["3-5 precise negative points/risks"],
    "driversToWatch": [{ "driver": "Name of Driver", "description": "Concise explanation of why it matters" }],
    "sentimentNarrative": "Short analysis of the tonality and confidence level.",
    "behavioralRead": "Short notes on management behavior and defensiveness.",
    "bullishCase": "Synthesis of the investment case for the upside (max 2 sentences).",
    "bearishCase": "Synthesis of the investment case for the downside (max 2 sentences).",
    "biggestRisk": "The single most concerning risk mentioned.",
    "biggestOpportunity": "The single most promising growth catalyst.",
    "finalTakeaway": "Final investor conclusion.",
  ...
  try {
    ...
    console.warn("[Gemini:JSON] Raw text might contain markdown:", rawText);
    try {
